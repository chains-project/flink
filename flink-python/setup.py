...
try:
    ...
    install_requires = ['py4j==0.10.9.7', 'python-dateutil==2.8.2', 'apache-beam==2.43.0',
                        'cloudpickle==2.2.0', 'avro-python3>=1.10.2,!=1.9.2,<1.10.3',
                        'pytz>=2018.3', 'fastavro>=1.7.0,<1.8.0', 'requests>=2.26.0',
                        'protobuf<4.22',
                        'pemja==0.2.6;'
                        'python_full_version >= "3.7" and platform_system != "Windows"',
                        'httplib2>=0.21.0,<=0.21.0', apache_flink_libraries_dependency]

    if sys.version_info < (3, 7):
        # python 3.6 upper and lower limit
        install_requires.append('numpy>=1.23.3,<1.24')
        install_requires.append('pandas>=1.5,<1.6')
        install_requires.append('pyarrow>=10.0.0,<10.1.0')
    else:
        # python 3.7, 3.8 and 3.9 upper limit and M1 chip lower limit,
        install_requires.append('numpy>=1.23.3,<1.24.0')
        install_requires.append('pandas>=1.5.0,<1.6.0')
        install_requires.append('pyarrow>=10.0.0,<10.1.0')
